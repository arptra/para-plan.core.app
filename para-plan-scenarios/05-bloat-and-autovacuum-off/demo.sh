...
psql "postgresql://paraplan:paraplan@localhost:5405/demo" -v ON_ERROR_STOP=1 -f init.sql

echo "---- SLOW PHASE ----"
echo "Trying EXPLAIN (may be slow or timeout)"
psql "postgresql://paraplan:paraplan@localhost:5405/demo" -v ON_ERROR_STOP=0 -c "SET statement_timeout='5s'; EXPLAIN (ANALYZE, BUFFERS) $(cat slow.sql)" || echo "OK: observed slowness/timeout"

#!/usr/bin/env bash
set -euo pipefail
echo "Bringing up Postgres on port 5403..."
docker-compose up -d
...
psql "postgresql://paraplan:paraplan@localhost:5403/demo" -v ON_ERROR_STOP=1 -f init.sql
...
echo "---- FIX PHASE ----"
psql "postgresql://paraplan:paraplan@localhost:5403/demo" -v ON_ERROR_STOP=1 -f fix.sql

#!/usr/bin/env bash
set -euo pipefail
echo "Bringing up Postgres on port 5402..."
docker compose up -d
...
psql "postgresql://paraplan:paraplan@localhost:5402/demo" -v ON_ERROR_STOP=1 -f init.sql
...
psql "postgresql://paraplan:paraplan@localhost:5402/demo" -v ON_ERROR_STOP=1 -c "EXPLAIN (ANALYZE, BUFFERS) $(cat slow.sql)"
